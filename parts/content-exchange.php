...
$compID = '';
if( isset($_POST['compid']) ) { // URLの引数にデータがある時
	$compID = $_POST['compid'];
}
// 自分のユーザーメタを取得
$myComps[0] = array();
...
<label for="comment">メッセージ本文</label>
...
<p><!--<?php echo EXCHANGE_NAME; ?>の相手：--><select name="compid" class="comp-name" id="compid" onchange="jQuery(function(){jQuery('#change-comp').submit();});">
...
<input type="submit" value="<?php echo EXCHANGE_NAME; ?>を表示する" style="display: inline-block;" onclick="$('div.exchange-cont').css('display', 'block');" class="submit"></p>
</form>
<?php
	if( empty($compID) ) :
?>
...
<p class="deleat-member"><a href="javascript:;">※退会した会員を一覧から削除する</a></p>
